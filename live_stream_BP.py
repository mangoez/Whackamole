...
class Graph:
    def __init__(self, board_shim):
        ...
        self.alpha_L = np.ones(self.partial_window_size * self.sampling_rate)/10
        self.alpha_R = np.ones(self.partial_window_size * self.sampling_rate)/10
        self.alpha_ratio = np.ones(self.partial_window_size * self.sampling_rate)/10
    ...
    def update(self):
        ...
        if data.shape[1] >= 2000:
            ...
            motor_all_bp_R = DataFilter.get_avg_band_powers(data[:, -1000:], [3], self.sampling_rate, False)
            motor_all_bp_L = DataFilter.get_avg_band_powers(data[:, -1000:], [4], self.sampling_rate, False)

            # For calculating beta band power from concentrating
            weighted_beta = EWA(self.beta[-1], front_all_bp[0][3], beta=0.95, threshold=0.15)
            ...
            weighted_alpha_L = EWA(self.alpha_L[-1], motor_all_bp_L[0][3], beta=0.98, threshold=0.8)
            self.alpha_L = np.hstack((self.alpha_L[1:self.num_points_partial], weighted_alpha_L))
            self.curves[1].setData(self.alpha_L)

            weighted_alpha_R = EWA(self.alpha_R[-1], motor_all_bp_R[0][3], beta=0.98, threshold=0.8)
            self.alpha_R = np.hstack((self.alpha_R[1:self.num_points_partial], weighted_alpha_R))
            self.curves[2].setData(self.alpha_R)

            alpha_ratio = EWA(self.alpha_ratio[-1], weighted_alpha_R/weighted_alpha_L, beta=0.99, threshold=1.5)
            self.alpha_ratio = np.hstack((self.alpha_ratio[1:self.num_points_partial], alpha_ratio))
            self.curves[3].setData(self.alpha_ratio)

            if weighted_beta > 0.11:
            ...
            print("Beta: ", weighted_beta, "Alpha R:", weighted_alpha_R, "Alpha L:", weighted_alpha_L, "Ratio of Alpha", alpha_ratio)                
